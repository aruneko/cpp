...
* C言語と違って、`return 0;`は必要ありません。自動で0を返します
...
* int型に変換することができ、`true`が`0`、`false`が`1`になります
* 逆にint型から変換すると、`0`が`true`、`0以外`が`false`になります
...
* boolalphaをcoutに挿入すると、それ以降のbool型の表示が`0`や`1`から`true`や`false`になります
